//===-- I8080InstPrinter.cpp - Convert I8080 MCInst to assembly syntax ----===//
//
...
//===----------------------------------------------------------------------===//
//
// This class prints an I8080 MCInst to a .s file.
//
//===----------------------------------------------------------------------===//
...
void I8080InstPrinter::printRegName(raw_ostream &OS, unsigned RegNo) const {
    OS << StringRef(getRegisterName(RegNo)).lower();
}

void I8080InstPrinter::printInst(const MCInst *MI, raw_ostream &O,
...
//           Kind == MCSymbolRefExpr::VK_I8080_LO ||
//           Kind == MCSymbolRefExpr::VK_I8080_HI);
//
        OS << SRE->getSymbol();
...
void I8080InstPrinter::printCondCode(const MCInst *MI, unsigned OpNum,
                                   raw_ostream &O) {
    const MCOperand &Op = MI->getOperand(OpNum);
    ISD::CondCode CC = (ISD::CondCode)Op.getImm();
    const char *Str = I8080::condCodeToString(CC);
    O << Str;
}

// Print a 'memsrc' operand which is a (Register, Offset) pair.
void I8080InstPrinter::printMemOperand(const MCInst *MI, int OpNum,
                                         raw_ostream &O) {
...
void I8080InstPrinter::printOperand(const MCInst *MI, unsigned OpNo,
                                  raw_ostream &O) {
